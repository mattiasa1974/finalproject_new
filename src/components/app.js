...
class App extends React.Component {
  ...
  this.updateStateWithLocalStorageData("games");
  this.updateStateWithLocalStorageData("final18");
  this.updateStateWithLocalStorageData("final14");
  this.updateStateWithLocalStorageData("final12");
  this.updateStateWithLocalStorageData("brons");
  this.updateStateWithLocalStorageData("final");

  // Updata tables based on data in localstorage
  const table = calculateResult(this.state.games);
  const groups = {}
  table.forEach(country => {
    if (!groups[country.group]) groups[country.group] = [];
...
updateStateWithLocalStorageData(stage) {
  const gamesFromLS = localStorage.getItem(stage);
  if (gamesFromLS) {
    const gamesFromLSArr = JSON.parse(gamesFromLS);
    const newGamesArray = [];

    if(Array.isArray(this.state[stage])) {
      this.state[stage].forEach(gameInState =>{
        const index = gamesFromLSArr.findIndex(gameInLS => gameInLS.matchId === gameInState.matchId)
        if (index > -1) {
          const newGameData = gamesFromLSArr[index];
          gameInState.awayScore = +newGameData.awayScore;
          gameInState.homeScore = +newGameData.homeScore;
          gameInState.homeTeam = newGameData.homeTeam;
          gameInState.awayTeam = newGameData.awayTeam;
        }

        newGamesArray.push(gameInState);
      });

      this.state["stage"] = newGamesArray;
    }
    else {
      const newGameData = JSON.parse(gamesFromLS)[0];
      const gameInState = this.state[stage];

      if(gameInState.matchId === newGameData.matchId) {
        gameInState.awayScore = +newGameData.awayScore;
        gameInState.homeScore = +newGameData.homeScore;
        gameInState.homeTeam = newGameData.homeTeam;
        gameInState.awayTeam = newGameData.awayTeam;
  ...
  handleScore18 = (matchId, awayTeam, awayScore,
    homeTeam, homeScore) => {
      ...
      this.addGameToLocalStorage("final18", game);
  ...
  handleScore14 = (matchId, awayTeam, awayScore,
    ...
    this.addGameToLocalStorage("final14", game);
  ...
  handleScore12 = (matchId, awayTeam, awayScore,
    ...
    this.addGameToLocalStorage("final12", game);
  ...
  handleScoreFinal = (matchId, awayTeam, awayScore,
    homeTeam, homeScore) => {
      ...
      this.addGameToLocalStorage("final", game);
  ...
  handleScoreBrons = (matchId, awayTeam, awayScore,
    homeTeam, homeScore) => {
      ...
      this.addGameToLocalStorage("brons", game);
  ...
  handleScore = (matchId, awayTeam, awayScore,
    homeTeam, homeScore) => {
      ...
      this.addGameToLocalStorage("games", game);
  ...
  addGameToLocalStorage(stageName, game) {
      // get all values in local storage for stage
      console.log("addGame", stageName, game)
      let stage = JSON.parse(localStorage.getItem(stageName)) || [];

      // remove (if it exist) game from stage
      this.remove(stage, game);

      // add game to stage
      stage.push(game);

      // rewrite stage to local storage
      localStorage.setItem(stageName, JSON.stringify(stage));
  }

  addFinal18ToLocalStorage(stageName, final18) {
      // get all values in local storage for stage
      console.log("addGame", stageName, final18)
      let stage = JSON.parse(localStorage.getItem(stageName)) || [];

      // remove (if it exist) game from stage
      this.remove(stage, final18);
  ...
  render() {
    ...
    return (
      ...
      <div className = "header-headline" >
        <h1> Vilka tar VM-guld? Hitta DIN vinnare </h1>
        <h3> Tippa alla matcher i mästerskapet </h3>
      </div>
